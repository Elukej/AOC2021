               ...
               std::unordered_set<Point, MyHashFunc> &paper_uset)
{
    for ( auto it1 = paper.before_begin(), it2 = paper.begin();
         it2 != paper.end(); )
    {   //na linijama presavijanja nema tarabi pa to ni nece biti ispitivano
...
int main(int argc, char** argv)
{
    ...
    while (std::getline(input, line)) 
        if (line.find('x') != std::string::npos) {
            ...
            fold.push_back(Point(0, luka_string::num_parser(line).front()));
            yfinal = fold.back().y;
        }

    for (auto &mem : fold) 
        foldPaper(mem, paper, paper_uset);
    std::cout << "Number of #: " << paper_uset.size() << std::endl;

    std::vector<std::vector<char>> code(yfinal, std::vector<char>(xfinal, '.'));
